package com.cebbys.cedyte.content.resources;
...
public class CedytePackGenerator {
	...
	public static void generateClientPack() {
		Artifice.registerAssetPack(new Identifier(Cedyte.MOD_ID, "resource_pack"), pack -> {
			...
			for (int i = 0; i < 8; i++) {
				final int index = i;
				pack.addBlockModel(id("template/tree/branch/" + String.valueOf(i)), model -> {
					generateBranchTemplateModel(model, index);
				});
				pack.addBlockModel(id("template/tree/root/" + String.valueOf(i)), model -> {
					generateRootTemplateModel(model, index);
				});
			}
			generateTreeModel(pack, "oak");
			generateTreeModel(pack, "spruce");
		});
	}
